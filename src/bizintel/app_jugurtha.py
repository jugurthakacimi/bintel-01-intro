...
from bizintel.utils_viz import plot_bar, plot_pie
...
def sales_by_region(
    ...
    # Jugurtha's Modifications
    # Add a Percentage column showing each region's share of total sales
    df_region["Percentage"] = (
        df_region["SaleAmount"] / df_region["SaleAmount"].sum() * 100
    )
...
def sales_by_category(
    ...
    # Jugurtha's Modifications
    # Add a Percentage column showing each category's share of total sales
    df_category["Percentage"] = (
        df_category["SaleAmount"] / df_category["SaleAmount"].sum() * 100
    )
...
def main() -> None:
    ...
    LOG.info("CALL a function to plot sales by region pie chart........")
    plot_pie(
        df=df_region,
        labels_col="Region",
        values_col="SaleAmount",
    ...
    LOG.info("CALL a function to plot sales by product category pie chart........")
    plot_pie(
        df=df_category,
        labels_col="Category",
        values_col="SaleAmount",
